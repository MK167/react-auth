...
### إعدادات الكوكي (`src/utils/cookie.service.ts`)

كوكيز الـ `accessToken` والـ `refreshToken` بيشتركوا في نفس الإعدادات:

| الإعداد | القيمة | السبب |
|---|---|---|
| `expires` | `7` أيام | بيخلي الـ session شغال حتى بعد إغلاق المتصفح وفتحه تاني |
| `secure` | `import.meta.env.PROD` | `true` في الإنتاج (HTTPS فقط)؛ `false` في التطوير. Safari بيرفض بصمت كتابة الكوكيز اللي فيها `Secure` على HTTP العادي — Chrome بيتساهل مع `localhost` وده السبب إن اللوجين كان بيفشل في Safari بس. |
| `sameSite` | `"lax"` | بيسمح بالكوكيز خلال top-level OAuth / Firebase redirects وفي نفس الوقت بيحمي من الـ cross-site POST requests. لو استخدمنا `"strict"` كان هيمنع الكوكي بعد ما الـ Firebase popup يرجع للتطبيق. |

> **ملاحظة Safari:** استخدام `secure: true` على `http://localhost` بيخلي Safari يتجاهل كل كتابة للكوكيز بصمت. الـ auth store بعدين بيلاقي مفيش token وبيمسح الـ session فوراً بعد اللوجين — ده اللي كان بيخلي كل طرق اللوجين تفشل في Safari بينما Chrome شغال عادي.
